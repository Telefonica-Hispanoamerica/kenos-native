...
import {Text4, Text6, Text7} from '../Text/Text';
...
const NavBar = ({
  ...
  const styles = StyleSheet.create({
    bigHeader: {
      height: displayMinimal ? 0 : (isIos() ? 62 : 53),
    ...
    textBig: {
      marginLeft: 16,
      marginRight: 16,
      marginTop: isIos() ? 'auto' : 8,
      marginBottom: isIos() ? 4 : 8,
      opacity: displayMinimal ? 0 : opacity as unknown as number,
      color: isIos() ? 'white' : undefined,
      fontWeight: isIos() ? '300' : undefined,
      lineHeight: isIos() ? 32 : undefined,
    },
    textSmall: {
      flex: 1,
      marginLeft: isIos() ? undefined : 12,
    ...
    leftButtonContainer: {
      marginLeft: isIos() ? 75 : 8,
    },
    rightButtonContainer: {
      flexDirection: 'row',
      marginRight: isIos() ? 10 : 8,
      gap: 8,
    },
  });
