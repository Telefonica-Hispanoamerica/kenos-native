...
const IconErrorGlobal = ({ size, color, skipAnimation }: IconErrorProps) => {
    return (
        <LottieView source={require('./animated/error.json')} autoPlay
            loop={false}
            resizeMode="cover"
            autoSize={true}
...
const IconErrorO2 = ({ size, color, skipAnimation }: IconErrorProps) => {
    return (
        <LottieView source={require('./animated/error-o2.json')} autoPlay
            loop={false}
            resizeMode="cover"
            autoSize={true}
            style={{ width: size, height: size }}
            ...
            colorFilters={[
                ...
                { keypath: "Capa de formas 2.Elipse 1.Trazo 1", color: color || '#eb3c7d' },
            ]}
        />
    );
}
...
function IconError({ size, color, skipAnimation }: IconErrorProps) {
    ...
    const errorLogo: ErrorLogoMap = {
        Movistar: <IconErrorGlobal size={size} color={color} skipAnimation={skipAnimation} />,
        'Movistar-AR': <IconErrorGlobal size={size} color={color} skipAnimation={skipAnimation} />,
        Vivo: <IconErrorGlobal size={size} color={color} skipAnimation={skipAnimation} />,
        O2: <IconErrorO2 size={size} color={color} skipAnimation={skipAnimation} />,
        Telefonica: <IconErrorGlobal size={size} color={color} skipAnimation={skipAnimation} />,
        BlauLogo: <IconErrorGlobal size={size} color={color} skipAnimation={skipAnimation} />,
    };
